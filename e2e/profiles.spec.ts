...
test.describe("Given a Profile link posted on a social media website/app", async () => {
  test.describe("When checking Open Graph meta tags", async () => {
    test("Then it should render the expected base-line meta tags", async ({ digiProfile }) => {
      await digiProfile.open();

      expect(await digiProfile.extractOpenGraphProperties()).toMatchObject({
        "og:title": `${digiProfile.handle} profile`,
        "og:description": "onchain social",
        "og:url": expect.stringContaining(`/u/${digiProfile.handle}`),
        "og:site_name": "DiGi Share",
        "og:type": "profile",
      });
    });

    test("Then it should include the expected Twitter Card meta tags", async ({ digiProfile }) => {
      await digiProfile.open();

      expect(await digiProfile.extractTwitterMetaTags()).toEqual({
        "twitter:card": "summary_large_image",
        "twitter:site": "DiGiProtocol",
        "twitter:title": `${digiProfile.handle} profile`,
